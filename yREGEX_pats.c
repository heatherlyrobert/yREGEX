...
yregex_pats__abbr_ref   (int *a_rpos)
{
   ...
   --rce;  if (x_ch != '&') {
      yregex_error_add (__FUNCTION__, a_rpos, "#PAT", 0, 0, "does not lead with a dollar sign");
      DEBUG_YREGEX  yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
   ...
   if (x_pat < 0) {
      yregex_error_add (__FUNCTION__, a_rpos, "#PAT", 0, 0, "not a valid pattern abbreviation");
      ++(*a_rpos);
...
yregex_pats__named_ref  (int *a_rpos)
{
   ...
   DEBUG_YREGEX  yLOG_value   ("x_ch"      , x_ch);
   --rce;  if (x_ch != '&') {
      yregex_error_add (__FUNCTION__, a_rpos, "#PAT", 0, 0, "does not lead with an ampersand");
      DEBUG_YREGEX  yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
   x_beg  = *a_rpos;
   ...
   /*---(troubles)-----------------------*/
   --rce;  if (x_len == 0) {
      yregex_error_add (__FUNCTION__, a_rpos, "#PAT", 0, 0, "could not parse name");
      DEBUG_YREGEX  yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
   --rce;  if (x_ch  != ')') {
      yregex_error_add (__FUNCTION__, a_rpos, "#PAT", 0, 0, "could not find close paren");
      DEBUG_YREGEX  yLOG_exitr   (__FUNCTION__, rce);
      return rce;
   }
   /*---(abbreviated name)---------------*/
   ...
   DEBUG_YREGEX  yLOG_value   ("x_pat"     , x_pat);
   if (x_pat < 0) {
      yregex_error_add (__FUNCTION__, a_rpos, "#PAT", 0, 0, "not a valid pattern name");
      *a_rpos += x_len;
      strlcat (myREGEX.regex, "#PAT", LEN_REGEX);
      DEBUG_YREGEX  yLOG_exitr   (__FUNCTION__, rce);
      return rce;
